...
int start_time, adc_rx_time, complete_time;
...
while(1)
	...
	if(stream)
	{
		complete_time = get_time_micros(T4);
		timer_start_micros(T4,50000);
		start_time = get_time_micros(T4);
		//Read the Analog data
		adc_multi_ch_rx(adc1, channels, adc_channels, analog_rx);
		adc_rx_time = get_time_micros(T4);
