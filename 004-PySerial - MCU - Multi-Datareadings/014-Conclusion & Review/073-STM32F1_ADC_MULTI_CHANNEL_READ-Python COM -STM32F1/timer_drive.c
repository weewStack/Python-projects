...
char servo_speed_CD(char port, char pin, int speed){
	...
	int speed_pwm = 1500 - speed*8;
	timer_pwm_micro(port,pin,21000,speed_pwm);
	return 1;
}
...
char esc_speed(char port, char pin, int speed){
	...
	int rpm_speed_pwm = 1000 + speed;
	timer_pwm_micro(port,pin,18000,rpm_speed_pwm);
	return 1;
}
/*********************************************************
...
void timer_capture_micro(char port, char pin, int arr, char EDGE){
	...
	if ((tim->CCER & (1 << ((channel-1)*4)) ) == 0)
	{
		...
		switch(channel)
		{
			case 1: tim->CCMR1 |= 0x1;break;
			case 2: tim->CCMR1 |= 0x100;break;
			case 3: tim->CCMR2 |= 0x1;break;
			case 4: tim->CCMR2 |= 0x100;break;
		}
		tim->CCER |= (1 << ((channel-1)*4));
		tim->BDTR |= 0x8000;
		tim->CR1 |=1;
...
int timer_get_capture_micro(char port, char pin){
	...
	switch(channel)
	{
		case 1: my_capture = tim->CCR1; break;
		case 2: my_capture = tim->CCR2; break;
		case 3: my_capture = tim->CCR3; break;
...
void sr04_init(char port_trig,char trigger_pin,char port_echo, char echo_pin){
	timer_pwm_micro(port_trig,trigger_pin,61000,10);
	timer_capture_micro(port_echo,echo_pin,61000,FALL);
}

int sr04_get_value(char port, char pin){
	int my_distance = (timer_get_capture_micro(port,pin)-469)/58;
	return my_distance;
}
